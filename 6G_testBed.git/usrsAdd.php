...
<?PHP
    ...
    if(isset($_REQUEST['addSubmit']))
    {
        ...
        $newSql       = "INSERT INTO usersInfoTbl VALUES('".$userID."','".$uname."','".$upwd."','".$umail."',now(),now())";
        $newSqlResult = mysql_query($newSql);
        if($newSqlResult)
        {
            mysql_free_result($newSqlResult);
            echo("<script>
                          alert('OK. Add User Info successfully!');
        ...
        else
        {
            mysql_free_result($newSqlResult);
            echo("<script>
                          alert('Oop. Add User Info Failure!');
                          window.location.href='usrsAdd.php';
                 </script>");
        }
    }
    mysql_close($connect);
...
<html>
    ...
    <body>
        ...
        <div style="font-family: fantasy; font-size: 16px; font-weight: bold; margin-left: 10px; color: white;">Modify User Info
            ...
            <table>
                <tr>
                    ...
                    <td style="width: 300px; font-family: fantasy; font-size: 12px; font-weight: normal;">
                        <output name="usrID" id="usrID" for="usrID" style="width: 299px; color: white; padding-left: 5px;">0</output>
                    </td>
            ...
            <button type="submit" class="submitBtn" name="addSubmit" id="addSubmit" onclick="return dataCheck()" value="addSubmit">Submit</button>
...
function genUsrID()
{
    ...
    {
        if(usrInfo[ulLoop].usrid == ulUsrID)
        {
            ulUsrID++;
        }
        else
    ...
    document.getElementById("usrID").value = ulUsrID;
    return;
}
